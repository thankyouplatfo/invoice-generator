...
@section('breadcrumb')
    الفواتير <a href="{{ route('invoices.create') }}" class=" w3-button w3-green w3-large m-3 w3-left"><i
            ...
            @foreach ($invoices as $customer)
                <tr>
                    ...
                    <td class="w3-center"><a
                            href="{{ route('invoices.show', $customer->id) }}">{{ $customer->name }}</a></td>
                    ...
                    <td class="w3-center"><a class="w3-button w3-bar-item w3-yellow"
                            href="{{ route('invoices.edit', $customer->id) }}"><i
                ...
                <form action="{{ route('invoices.destroy', $customer->id) }}" id="del_customer" method="post">@csrf
                    @method('DELETE')
                </form>
            @endforeach
        </table>
